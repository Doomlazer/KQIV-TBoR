		...
		(if (== (ego script?) 0)
			(cond 
				...
				((& (ego onControl: TRUE) $0001)
					(if (== (ego view?) 2)
						(ego setScript: slide) ;added to prevent freezing when climbing down
						(fallSound play:) ;added sound effect
						(slide changeState: 1)
					else
