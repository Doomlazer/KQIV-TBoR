...
(instance Room57 of Room
	...
	(method (init)
		...
		(if (> gamePhase getTheHen)
			...
			(if (ego has: iWitchGlassEye)
				(= witchesEyeless TRUE)
				(if (and (not (ego has: iObsidianScarab)) (not witchesTossedScarab))
					(Load SCRIPT JUMP)
					((= scarab (Actor new:))
						view: 631
						loop: 3
	...
	(method (doit)
		(cond 
			...
			((> gamePhase getTheHen)
				(cond 
					((and (ego has: iWitchGlassEye) (!= (witch3 script?) witchMoan))
	...
	(method (handleEvent event &tmp index)
		...
		(if (> gamePhase getTheHen)
			(cond 
					...
					(cond 
						...
						((Said '/hag')
							(cond 
								...
								(((inventory at: iWitchGlassEye) ownedBy: -1)
									(Print 57 6)
								)
								(else (Print 57 5))
							)
				...
				((Said 'converse')
					(cond 
						(((inventory at: iWitchGlassEye) ownedBy: 57)
							(answer1 cue:)
						)
						((ego has: iWitchGlassEye)
							(answer2 cue:)
						)
						(else
							(answer3 cue:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(Print {It wards off the undead, obviously.} #draw #icon 403 0 0)
				(= gotItem TRUE)
