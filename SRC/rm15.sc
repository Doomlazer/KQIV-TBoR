				...
				(cond 
					...
					((Said 'get,capture/frog')
						(cond 
							...
							((ego has: iGoldBall) (PrintCantDoThat))
							((ego inRect: 249 133 266 139) (Print 15 29))
							((== frogPrinceState 7) (PrintCantDoThat))
					...
					((Said 'eat/frog')
						(if (== frogPrinceState frogHeldByEgo)
								...
								(frogActions changeState: 666)		
							
						else  
								(Print {You'd need to be holding the frog to do that!})
					...
					((Said 'get,rob/crown')
						(cond 
							...
							((== frogPrinceState frogHeldByEgo)
								(Print 15 46 #icon 402 0 0 #dispose)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(13
				...
				(AnimateCast)
				(Print 15 48 #icon 432 0 0 #dispose)
				(= frogPrinceState frogHeldByEgo)
				(HandsOn)
			)
			(50
			...
			(667
					(ego view: 30 loop: 1 cel: 0 cycleSpeed: 4 setCycle: EndLoop)
					(ego get: iSmallCrown)
					(= seconds 4)
			)
			(668
				(if (ego has: iTooth)
					(Print 15 55 #icon 402 0 0 #draw)
				else
					(Print 15 56  #draw #icon 402 0 0)
				)
				((Inventory at: iSmallCrown) moveTo: ego)
				(= gotItem TRUE)
				(ego
