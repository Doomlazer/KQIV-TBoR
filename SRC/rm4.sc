...
(instance Room4 of Room
	...
	(method (init)
		...
		(if (and (== gamePhase getTheHen) (not ogressIsHome))
			...
			(ogress setMotion: MoveTo 103 168 ogressActions)
	...
	(method (doit)
		...
		(if (cast contains: ogress)
			(cond 
				((ogress inRect: 143 0 369 136) (ogress setPri: 1))
				((< (ogressActions state?) 200) (ogress setPri: -1))
			)
		)
				...
				(and
					(not
						(if (ego inRect: 134 0 369 137)
							(ogress inRect: 104 149 369 185)
						)
					)
					(< (ego distanceTo: ogress) 75)
				)
		...
		(if
			(and
				...
				(not (ogress inRect: 104 149 369 185))
			)
			(= local4
				(GetAngle (ogress x?) (ogress y?) (ego x?) (ego y?))
			)
			(if
				(and
					(< (= newActHeading (ogress heading?)) 15)
					(> local4 345)
				)
				...
				(cond 
					((Said 'look>')
						(cond 
							...
							((Said '/buck') (if (cast contains: ogress) (Print 4 4)))
							((Said '/window')
					...
					(
					(and (cast contains: ogress) (Said 'converse'))
						(if (and (== (ogress view?) 246) (!= ogressState 3))
					...
					((Said 'get,capture/buck')
						(cond 
							((cast contains: ogress) (Print 4 13))
	...
	(method (newRoom newRoomNumber)
		...
		(= noWearCrown 0)
		(if (cast contains: deer) (deer dispose:))
		(if (cast contains: ogress) (ogress dispose:))
...
(instance ogressActions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0 (if ogress (= seconds 3)))
			(1
				(= ogressState ogressAppears)
				(ogress setLoop: 0)
			)
			(2
				(ogress setMotion: MoveTo 183 155 ogressActions)
			)
			...
			(10
				...
				(deer
					view: 530
					ignoreActors:
					setPri: (+ (ogress priority?) 1)
					posn: (- (ogress x?) 6) (+ (ogress y?) 3)
					setCycle: EndLoop self
					init:
				)
				(ogress setLoop: -1 setMotion: 0 view: 245)
				(door setCycle: 0)
			)
			(11
				(ogress
