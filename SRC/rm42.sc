	...
	(method (init)
		...
		(if (ego has: iDiamondPouch)
			...
			(if
				...
				(if (not wifeDead)((= wife (Actor new:))
					view: 240
					illegalBits: 0
					loop: 2
					posn: 84 132
			...
			(if (not wifeDead)((= wifeSitting (Actor new:))
				view: 240
				loop: 4
				cel: 1
				illegalBits: 0
			...
			(if (== (event type?) saidEvent)
				(cond 
								...
								(if (cast contains: fisherman)	
									(if wifeDead 
										...
										(if ((inventory at: iFishingPole) ownedBy: 204)
											(fisherman setScript: givePoleBreasts)
										else
											(Print {"Nice," says the fisherman.})	
										)
									)
								else 
									(Print 42 77)
					...
					((Said 'look[<around][/room,room,place]')
						(if shouldknowwifedead (Print 42 0) else
						(if wifeAtCounter
							(Print
								(Format @str 42 0
									(cond 
										((not (cast contains: fisherman))
											{Sure these people are poor, but at least they have a house. Your dad makes the peasants in Danventry live in the sewers.}
										)
					...
					(
						(or
							...
							(and
								...
								;;this was crashing when talking to the man if the wife is dead. null pointer fix:
								(if (not wifeDead))
...
(instance givePoleBreasts of Script
	(properties)
...
(instance wifeTacoDoco1 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if talkedToFishermanWife (self cue:) (return))
				(if deadswitch (Print 42 76) else (Print 42 62))
				(= talkedToFishermanWife 1)
			)
			(1 (= state 0) (if deadswitch (Print 42 76) else (Print 42 63) ))
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(= talkedToFishermanWife -1)
				(if deadswitch (Print 42 76) else (Print 42 64) )
			)
			(1
				(= state 0)
				(Print
